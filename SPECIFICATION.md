...
**This is the flagship unit** — local book-state maintenance plus checksum recomputation is the strongest technical demonstration in the project. The implementer must follow the checksum algorithm from the Bitfinex documentation and must document it in `docs/`. The numeric-token wire contract (plain-decimal serialisation of each price/amount, including exponent-form values such as `1e-8`) is defined in `docs/adr/ADR-007-checksum-numeric-token-contract.md`.
